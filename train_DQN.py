...
if __name__ == '__main__':
 # PDQN=====================================================================
    ...
    save_dir ="results_DQN_5v3" #Output directory
    load = False 
    load_dir ="results/"+title+"0"
    load_num="999"
    threshold = 0.005#1e-3
    start_episode=0
    MAXepisode = 80000#600#20000
    MAXstep = 100#150
    ...
    while cont: # episode
        ...
        if save_freq > 0 and save_dir and episode % save_freq == 0:
            agentDQN.save_models(os.path.join(save_dir, str(episode)))
        # reset  
        s = env.reset()
        s = np.array(list(s), dtype=np.float32, copy=False)
        # 1) take an action----------------------------------------------------
        a1 = agentDQN.act(s)   
        c1,P1=agentDQN.action_decoder(a1, env.P_Max_SBS)
        ...
        while True:  # step 
            ...
            # 3) take an action------------------------------------------------
            a1_ = agentDQN.act(s_)
            c1_,P1_=agentDQN.action_decoder(a1_, env.P_Max_SBS)
            # 4) learn---------------------------------------------------------
            agentDQN.step(s, (a1), R1, s_,  (a1_), done1 )
    ...
    # save model
    if save_freq > 0 and save_dir:
        agentDQN.save_models(os.path.join(save_dir, '_done'))   
